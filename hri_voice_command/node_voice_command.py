...
def _candidate_config_paths() -> List[str]:
    """Return config lookup locations in priority order."""
    paths: List[str] = []

    env_path = os.environ.get("HRI_VOICE_COMMAND_CONFIG")
    if env_path:
        paths.append(env_path)

    try:
        from ament_index_python.packages import get_package_share_directory
        share_dir = get_package_share_directory("hri_voice_command")
        paths.append(os.path.join(share_dir, "config", "config.yaml"))
    except Exception:
        pass

    script_dir = os.path.dirname(os.path.realpath(__file__))
    paths.append(os.path.join(script_dir, "config.yaml"))
    paths.append(os.path.join(os.getcwd(), "config.yaml"))
    return paths


def _load_voice_config() -> Tuple[dict, str]:
    ...
    if last_error:
        print(f"⚠️ Error loading config.yaml: {last_error}")
    else:
        print("⚠️ config.yaml not found; using built-in defaults")
    return {"VOICE_COMMANDS": {"start system": 0, "stop system": 8}, "WAKE_WORDS": ["kuka"]}, "<built-in defaults>"


config, yaml_full_path = _load_voice_config()
...
class WhisperROS2Node(Node):
    def __init__(self, args: argparse.Namespace):
        super().__init__("hri_voice_command")
        self.args = args

        # Legacy ROS2 publishers kept for backward compatibility with the original script.
        ...
        # ROS4HRI-compatible voice topics.
        # Standard topics:
        #   /humans/voices/tracked
        #   /humans/voices/<voice_id>/speech
        #   /humans/voices/<voice_id>/is_speaking
        # Command topics are an application-specific extension under the same voice namespace.
        self.hri_voice_id = args.hri_voice_id
        self.hri_voice_ns = f"/humans/voices/{self.hri_voice_id}"
        self.hri_voice_tracked_pub = self.create_publisher(IdsList, args.hri_tracked_voices_topic, 10)
        self.hri_speech_pub = self.create_publisher(
            LiveSpeech, args.hri_speech_topic or f"{self.hri_voice_ns}/speech", 10
        )
        self.hri_is_speaking_pub = self.create_publisher(
            Bool, args.hri_is_speaking_topic or f"{self.hri_voice_ns}/is_speaking", 10
        )
        self.hri_command_pub = self.create_publisher(
            String, args.hri_command_topic or f"{self.hri_voice_ns}/command", 10
        )
        self.hri_command_id_pub = self.create_publisher(
            Int32, args.hri_command_id_topic or f"{self.hri_voice_ns}/command_id", 10
        )
        self.create_timer(1.0, self.publish_hri_tracked_voice)
    ...
    def publish_hri_tracked_voice(self):
        msg = IdsList()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.ids = [self.hri_voice_id]
        self.hri_voice_tracked_pub.publish(msg)

    def publish_hri_is_speaking(self, is_speaking: bool):
        msg = Bool()
        msg.data = bool(is_speaking)
        self.hri_is_speaking_pub.publish(msg)

    def publish_hri_speech(self, final_text: str, confidence: float = 1.0):
        msg = LiveSpeech()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.header.frame_id = f"voice_{self.hri_voice_id}"
        msg.incremental = ""
        msg.final = final_text
        msg.confidence = float(max(0.0, min(1.0, confidence)))
        msg.locale = self.args.hri_locale
        self.hri_speech_pub.publish(msg)

    def publish_hri_command(self, command_text: str, cmd_id: int):
        cmsg = String()
        cmsg.data = command_text
        self.hri_command_pub.publish(cmsg)

        imsg = Int32()
        imsg.data = int(cmd_id)
        self.hri_command_id_pub.publish(imsg)
    ...
    def publish_command(self, full_text: str, command_text: str, cmd_id: int, matched_phrase: str, score: float):
        ...
        self.publish_hri_speech(full_text, confidence=score)
        self.publish_hri_command(command_text, cmd_id)
            ...
            if self.state == "LISTEN":
                if (now - self.last_wake_probe_t) >= self.wake_hop_s and self._listen_gate_open():
                    ...
                    if has_wake:
                        ...
                        self.publish_hri_is_speaking(True)
                        self.cmd_buf = []
                        self.trailing_nonspeech_frames = 0
                ...
                if stop_on_silence or stop_on_max:
                    ...
                    self.publish_hri_is_speaking(False)
...
def main():
    ...
    # ROS4HRI-compatible voice topics
    parser.add_argument("--hri-voice-id", type=str, default="anonymous_speaker", help="ROS4HRI voice ID")
    parser.add_argument("--hri-locale", type=str, default="en_US", help="ROS4HRI LiveSpeech locale")
    parser.add_argument("--hri-tracked-voices-topic", type=str, default="/humans/voices/tracked")
    parser.add_argument("--hri-speech-topic", type=str, default="", help="Defaults to /humans/voices/<voice_id>/speech")
    parser.add_argument("--hri-is-speaking-topic", type=str, default="", help="Defaults to /humans/voices/<voice_id>/is_speaking")
    parser.add_argument("--hri-command-topic", type=str, default="", help="Extension topic; defaults to /humans/voices/<voice_id>/command")
    parser.add_argument("--hri-command-id-topic", type=str, default="", help="Extension topic; defaults to /humans/voices/<voice_id>/command_id")
